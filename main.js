...
var creepsRoles = require('creeps.roles');
const towersRoles = require("towers.roles");
...
function clearAll() {
    ...
    for (var i in Memory.creeps) {
        let type = Memory.creeps[i].type;
        let room = Memory.creeps[i].room;
...
function recountCreeps(){
    for (let roomName in Memory.rooms) {
        let memRoom = Memory.rooms[roomName];
        for (let creepType in memRoom.creeps) {
            memRoom.creeps[creepType].count = 0;
        }
    }
    for (let creepName in Memory.creeps) {
        let memCreep = Memory.creeps[creepName];
        let memRoom = Memory.rooms[memCreep.room];
        memRoom.creeps[memCreep.type].count ++;
...
module.exports.loop = function () {
    ...
    // for (let creepName in Memory.creeps) {
    //     Memory.creeps[creepName].type = "W";
    // }
    // for (let creepType in CreepType) {
    //     Memory.spawns["Spawn1"].creeps [CreepType[creepType]].count = 0;
    // }
    //
    // for (let creepName in Memory.creeps) {
    //     Memory.spawns["Spawn1"].creeps [Memory.creeps[creepName].type].count ++
    // }


    creepsRoles.processAllCreeps();
    for(var room in Game.rooms){
        var towers = Game.rooms[room].find(FIND_MY_STRUCTURES, {filter: {structureType: STRUCTURE_TOWER}});
        for(var id in towers){
            towersRoles.run(towers[id]);
        }
    }
    //
    // for (let roomName in Memory.rooms) {
    //     Memory.rooms[roomName].creeps['E'].need = 0;
    //     Memory.rooms[roomName].creeps['E'].max = 0;
    // }
}