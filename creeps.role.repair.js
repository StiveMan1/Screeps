...
function GetTasks(memRoom, gameRoom, roleTasks, memTasks, roomCreeps) {
    ...
    // STRUCTURE_CONTAINER
    if (memRoom.mode === RoomMode.Peaceful && ![RoomType.Friends, RoomType.My].includes(memRoom.type)) {
        // STRUCTURE_STORAGE
        ...
        _.forEach(targets, (target) => {
            task = new CreepsTask("", RoleName, [CreepType.Miner], target.pos);
            ...
            memTask.data.total = target.hitsMax;
            memTask.data.progress = target.hits;
        });
    }
}
...
function tryToWork(task, gameCreep, gameObj) {
    ...
    return gameCreep.repair(gameObj);
}
